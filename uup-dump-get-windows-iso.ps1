...
function Get-UupDumpIso($name, $target) {
    if ($name -eq '24H2') {
        ...
        return [PSCustomObject]@{
            ...
            build = '26100.7628'   # optional: fake build number
            id = $id
            edition = $target.edition
            virtualEdition = $target.virtualEdition
            apiUrl = "https://api.uupdump.net/get.php?id=$id&lang=en-us&edition=$($target.edition)"
            downloadUrl = "https://uupdump.net/download.php?id=$id&pack=en-us&edition=$($target.edition)"
            downloadPackageUrl = "https://uupdump.net/get.php?id=$id&pack=en-us&edition=$($target.edition)"
        }
    }
